...
一条白色杯子任务展示了从地图实例到实际操作的完整过程。用户站在物品台前约2 m，注视白杯1并通过语音指代该杯子；旁边约17 cm处放置另一只同款白杯2。机器人从房门外出发，初始位置看不到物品台。系统先将用户指代解析为地图中的白杯1，机器人随后进入房间，在近距离观测到两个杯子后，利用已保存的目标空间参照选中白杯1，并完成抓取和递送。

【插入执行过程图：用户注视与指令 → 地图中的目标实例 → 机器人从门外接近 → 区分同款杯子 → 抓取与递送。】

该案例表明，用户视角下确定的实例参照可以在机器人随后获得不同视角的观测时继续用于目标选择，从而支持目标初始不在机器人视野内的操作任务。
