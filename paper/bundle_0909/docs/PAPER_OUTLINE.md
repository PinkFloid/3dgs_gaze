...
锥后验在带标签高斯上投票(输出 top-k + p_none);候选集=命名实例+背景
(closed-world:命名表即可指代词汇表),并叠加**可交互性先验**(graspability
prior:家具类降权,**类内同权**——同类实例共享权重,实例间 argmax 不受影响,
仅抑制跨类漏票;静态、采集前定死,E4 消融)。
