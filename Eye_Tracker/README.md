...
| 工具 | 作用 |
|---|---|
| `gaze_live.py` | **实时模式**:单进程跑完 定位→gaze→世界→注视聚类→锥判定,cv2 窗口实时叠加(绿十字=原始视线、蓝点=修正后射线、物体框、判定+世界坐标);连续盯同一 tag ≥0.8s 才自动重估 bias/σ，断点/换 tag 会重置且用 MAD 剔除异常点；实时聚类同时按 15cm 半径与 0.8m/s 跳变切簇；注视中出暂定判定(黄'~')、移开定案(红'->');`--publish` 发 ZMQ `gaze.intent`(带 provisional 标志);`--replay <录像>` 无硬件回放;`--dump-video` 导出演示视频 |
