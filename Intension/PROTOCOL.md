...
| skill | params | 语义 |
|---|---|---|
| `grasp` | `object_name: str \| null`, `target_world: [x,y,z]` 米, `deliver_to: [x,y,z]`(可选) | **唯一技能,两种用法**:①`object_name` 有值 = 走到 standoff→对准→抓取,有 `deliver_to`(=确认时刻用户头位置)则送达、无则原地 done;②**`object_name = null` = 纯导航**,只走到 `target_world`,不动臂,进度 `moving→done` |
