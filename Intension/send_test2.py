...
坐标三元组 = x,y,yaw(米,米,弧度):第三位就是站位朝向,协议里不传高度。

    python send_test2.py                              # 默认打狗机
    python send_test2.py --obj apple --t1 2.27,-0.27,1.57
...
def xyw(s):
    ...
    assert len(v) == 3, "坐标格式 x,y,yaw(弧度)"
    return v
...
def main():
    ...
    ap.add_argument("--t1", type=xyw, default=[-1.37, 0.33, 2.52],
                    help="步骤1 站位 x,y,yaw(弧度)")
    ap.add_argument("--t2", type=xyw, default=[0.0, 0.0, 0.0],
                    help="步骤2 导航站位 x,y,yaw(弧度)")
    args = ap.parse_args()
    ...
    rep = send(ctx, args.host, args.rep, {
        **base, "req_id": f"test2-{tag}-1", "sent_at": time.time(),
        "skill": "grasp",
        "params": {"object_name": args.obj, "target_world": args.t1},
